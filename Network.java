...
public class Network extends Thread {
    ...
    private static int inputIndexClient, inputIndexServer, outputIndexServer, outputIndexClient; // Network buffer
                                                                                                 // indices for
                                                                                                 // accessing the input
                                                                                                 // buffer
                                                                                                 // (inputIndexClient,
    ...
    private static String inBufferStatus, outBufferStatus; // Current status of the network buffers - normal, full,
                                                           // empty
    ...
    Network() {
        ...
        outBufferFullSemaphore = new Semaphore(0);
        outBufferEmptySemaphore = new Semaphore(maxNbPackets);
    ...
    public static boolean send(Transactions inPacket) {
        try {
            ...
            inBufferFullSemaphore.release(); /* Release the empty semaphore for the input buffer */
            return true;
    ...
    public static boolean receive(Transactions outPacket) {
        try {
            outBufferFullSemaphore.acquire(); /* Acquire the full semaphore for the output buffer */
            outBufferMutex.acquire(); /* Acquire the mutex for the output buffer */
            ...
            outBufferMutex.release(); /* Release the mutex for the output buffer */
            outBufferEmptySemaphore.release(); /* Release the full semaphore for the output buffer */
    ...
    public static boolean transferOut(Transactions outPacket) {
        try {
            outBufferEmptySemaphore.acquire(); /* Acquire the empty semaphore for the output buffer */
            outBufferMutex.acquire(); /* Acquire the mutex for the output buffer */
            ...
            outBufferMutex.release(); /* Release the mutex for the output buffer */
            outBufferFullSemaphore.release(); /* Release the empty semaphore for the output buffer */
            return true;
        } catch (InterruptedException e) {
            e.printStackTrace();
            return false;
    ...
    public static boolean transferIn(Transactions inPacket) {
        try {
            ...
            inBufferMutex.release(); /* Release the mutex for the input buffer */
            inBufferEmptySemaphore.release(); /* Release the full semaphore for the input buffer */
            return true;
